...
async def send_code(
    ...
    if response:
        ...
        return json_response(status_code=400, message="User is already register.", scores=sum(scores))
    ...
    if response_send['status'] == 'error':
        raise HTTPException(status_code=500, message=f"Zvonok API Error: {response_data}")
